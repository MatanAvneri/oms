...
interface AppState {
  ...
  website: string,
  logo?: string,
  setData: (values: { name: string, role: string, phone: string, website: string, logo: string }) => void
  setValue: (name: 'name' | 'role' | 'phone' | 'website' | 'logo', value: string) => void,
...
const useStore = create<AppState>(
  immer((set) => ({
    ...
    setStyle: (values) => set((state: AppState) => {
      state.styles = {
        ...state.styles,
        ...values
      }
